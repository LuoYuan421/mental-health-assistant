        ...
        <div class="sidebar">
            <!-- AI助手信息 -->
             <div class="ai-assistant-info">
                ...
                <h3 class="assistant-name">小药AI助手</h3>
                ...
                <div class="message-item ai-message" v-if="messages.length === 0">
                    ...
                    <div class="message-content">
                        <div class="message-bubble">
                            <p>您好！我是小药，您的AI心理健康助手。很高兴陪伴您，为您提供温暖的心理支持。请告诉我，今天您感觉怎么样？有什么想要分享的吗？</p>
                        </div>
...
const loadSessionEmotion = (sessionId) => {
   // 确保sessionID格式正确
    ...
    getSessionEmotion(id).then(res => {
        console.log(res)
        try {
            const analysis = typeof res.emotionAnalysis === 'string' ? JSON.parse(res.emotionAnalysis) : res.emotionAnalysis
            if (analysis && typeof analysis === 'object' && Object.keys(analysis).length > 0) {
                currentEmotion.value = { ...defaultEmotion, ...analysis }
            } else {
...
const startNewSession = (message) => {
    ...
    if (currentSession.value.sessionTitle === '新对话') {
        sessionParams.sessionTitle = `小药AI助手 - ${new Date().toLocaleString()}`
    } else {
        // 如果历史会话记录
    ...
    fetchEventSource('/api/psychological-chat/stream', {
        ...
        onmessage: (event) => {
            ...
            if (eventName === 'done') {
                ...
                ctrl.abort()
                // 延迟读取情绪数据，等后端异步分析完成
                setTimeout(() => loadSessionEmotion(currentSession.value.sessionId), 4000)
                return
            }
